...
sub plot {
  ...
  system "gmt set PS_PAGE_ORIENTATION portrait";
  @trace = ("1/255/255/255","3/0/0/0");       # plot data trace
  @name = ("Pz","Pr","Sz"," Sr","Sh");
  
  ($nn,$hight) = (12,10.5);	# 24 rows of traces per 10.5 in.

  
  $sepa = 0.1*$sec_per_inch;
  ($tt, $inc) = (2*$t1 + 3*$t2 + 5*$sepa, 1);
  ...
  $plt1 = "| gmt pssac -JX$width/$hight -R0/$tt/0/$nn -Y0.2 -Ent-2 -M$am -K >> $outps";
  $plt2 = "| gmt pstext -JX -R -N -O -K  >> $outps";
  $plt3 = "| gmt psmeca -JX1/1 -R-1/1/-1/1 -Sa5 -G100 -Y10.0 -X-0.7 -O -K >> $outps";
  $plt3 = "| gmt psmeca -JX1/1 -R-1/1/-1/1 -Sm8 -G100 -Y10.0 -X-0.7 -O -K >> $outps" if $tensor[1] eq "MomentTensor";
  $plt4 = "| gmt psxy -JPa1 -R0/360/0/1 -Sx0.1 -W0.5p,red -O >> $outps";
  ...
  while (@rslt) {
	...
	if ($aa[4*$j+2]>0) {
	   printf PLT "%s %f %f 0/0/0\n",$nam.$com1,$x,$nn-$i-2;
           printf PLT "%s %f %f 255/0/0\n",$nam.$com2,$x,$nn-$i-2;
	} elsif ($keepBad) {
